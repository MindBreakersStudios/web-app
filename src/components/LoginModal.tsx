...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t(isLogin ? 'auth.login.error' : 'auth.register.error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDiscordLogin = async () => {
    ...
    try {
      ...
      setError(t('auth.social.discordError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSteamLogin = async () => {
    ...
    try {
      ...
      setError(t('auth.social.steamError'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-md relative">
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1">
                {t(isLogin ? 'auth.login.email' : 'auth.register.email')}
              </label>
              <input 
                type="email" 
                value={email}
                ...
                className="w-full bg-gray-900 border border-gray-700 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                placeholder={t(isLogin ? 'auth.login.emailPlaceholder' : 'auth.register.emailPlaceholder')}
                required
                disabled={loading}
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1">
                {t(isLogin ? 'auth.login.password' : 'auth.register.password')}
              </label>
              <input 
                type="password" 
                value={password}
                ...
                className="w-full bg-gray-900 border border-gray-700 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                placeholder={t(isLogin ? 'auth.login.passwordPlaceholder' : 'auth.register.passwordPlaceholder')}
                required
                minLength={6}
                disabled={loading}
              />
            ...
            <button 
              ...
              disabled={loading}
            >
              {loading ? t(isLogin ? 'auth.login.processing' : 'auth.register.processing') : (isLogin ? t('auth.login.submit') : t('auth.register.submit'))}
            </button>
          </form>
